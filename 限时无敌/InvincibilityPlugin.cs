...
namespace InvincibilityPlugin
{
    ...
    public class InvincibilityPlugin : TerrariaPlugin
    {
        ...
        {
            Commands.ChatCommands.Add(new Command("限时无敌", InvincibleCommand, "tempinv", "限时无敌"));
        }

        private void InvincibleCommand(CommandArgs args)
        {
            // 检查权限
            if (!args.Player.HasPermission("限时无敌"))
            {
            ...
            {
                args.Player.SendErrorMessage("用法: /限时无敌或tempinv <持续时间秒数>");
                return;
            }
        ...
        private void addGodMode(TSPlayer player)
        {
            ...
            if (Config.EnableInvincibleReminder == true)
            {
